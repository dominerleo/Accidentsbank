...
export default function PublicSafetyPopup({ item, onClose }: Props) {
  ...
  return (
    ...
    >
      <div
        className="min-w-[240px] max-w-[300px] rounded-lg border border-violet-200 bg-white p-3 text-sm shadow-xl"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="mb-2 flex items-start justify-between gap-2">
          <span className="rounded-full bg-violet-100 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-violet-700">
            {t.psPopupCategoryLabel}
          </span>
          <button
            type="button"
            onClick={onClose}
        ...
        <p className="text-[11px] text-slate-500">{t.psLayerTitle}</p>
