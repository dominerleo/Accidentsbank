...
- **한국어 / English**: 상단 탭으로 전환. 한국어는 카카오맵, English는 OpenStreetMap 기반 세계 지도와 영문 메뉴(지명 검색은 Nominatim).
- **개발 전략**:
...
### 2.2 사고 기록 시스템
- **기록(Deposit)**: 분류(사고·범죄·뉴스·이벤트·기타), 날짜, 내용, 뉴스 링크를 직접 등록.
- **시각화**: 카테고리별 맞춤형 아이콘(`lucide-react`) 마커 표시.
...
- **Icon**: `lucide-react`
- **Database**: Supabase (PostgreSQL + PostGIS), `@supabase/ssr` 세션
- **AI**: OpenAI API, Tavily Search API

## 4. 시작하기
```bash
npm install
cp .env.local.example .env.local
# .env.local: 카카오 키 + Supabase URL/키 (예시 파일 주석 참고)
npm run dev
```
브라우저에서 [http://localhost:3002](http://localhost:3002) 접속. (`npm run dev` 기본 포트는 3002)
