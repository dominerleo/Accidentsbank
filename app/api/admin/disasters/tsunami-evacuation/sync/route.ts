...
import {
  fetchTsunamiEvacuationRaw,
  normalizeTsunamiEvacuationItem,
...
const TSUNAMI_CATEGORY = "tsunami_evacuation_site";
/** 30 일 캐시 만료 (수시 갱신 데이터지만 보수적으로). */
...
function buildRow(
  ...
  // displayAddress 앞에 명칭이 있으면 함께 표시 (개인정보 아님 — 시설명).
  const display =
    item.name && item.name.trim()
      ? `${item.name.trim()} (${item.displayAddress})`
      : item.displayAddress;
  // 응답 좌표가 직접 들어 있으면 우선 사용. 없을 때만 카카오 지오코딩 결과 사용.
 ...
 *
 * safetydata.go.kr → 좌표 직접 사용(또는 카카오 지오코딩 보조) → public_safety_address_cache 저장.
 *
 * 본문(선택) 또는 쿼리:
 *   - pageNo: 1 이상 (기본 1)
 *   - numOfRows: 1..200 (기본 50)
 *
 * 응답:
 *   { ok: true, fetched, upserted, geocodeFailed, skipped, hadCoords, pageNo, numOfRows, totalCount }
 *
 * 권한: profiles.role IN ('admin','moderator') 또는 ADMIN_EMAILS 포함 사용자만.
 *
 * 보안: SAFETYDATA_API_KEY 는 응답·로그·에러메시지 어디에도 노출하지 않는다.
 */
export async function POST(req: Request): Promise<NextResponse> {
  ...
  try {
    raw = await fetchTsunamiEvacuationRaw({
      serviceKey,
      pageNo,
      numOfRows,
    });
    ...
    console.error("[disasters/tsunami-evacuation/sync] fetch failed");
    return NextResponse.json(
  ...
  const normalizedAll = parsed.items.map((rawItem, index) =>
    normalizeTsunamiEvacuationItem(rawItem, pageNo, index)
  );
  ...
  // partial unique index 회피: select-then-insert/update 패턴
  const keys = deduped
  ...
  const { data: existingRows, error: selErr } = await supabase
    ...
    .eq("source_type", "safetydata_tsunami_evacuation")
    .in("source_record_key", keys);
  if (selErr) {
    console.error(
      "[disasters/tsunami-evacuation/sync] select failed:",
      selErr.message
    );
    return NextResponse.json(
  ...
  const insertRows = deduped.filter(
  ...
  if (insertRows.length > 0) {
    ...
    if (insErr) {
      console.error(
        "[disasters/tsunami-evacuation/sync] insert failed:",
        insErr.message
      );
      return NextResponse.json(
  ...
  for (const r of updateRows) {
    ...
    if (updErr) {
      console.error(
        "[disasters/tsunami-evacuation/sync] update failed:",
        updErr.message
      );
      continue;
    }
