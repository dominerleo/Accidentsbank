/**
 * 지진해일 대피지구(safetydata.go.kr) 캐시 수동 동기화 스크립트.
 *
 * 사용 예:
 *   npx tsx scripts/sync-tsunami-evacuation.ts --pageNo=1 --numOfRows=200
 *   npx tsx scripts/sync-tsunami-evacuation.ts --pages=10 --numOfRows=200   # 1..10 페이지 순차 수집
 *
 * 환경(.env.local):
 *   - SAFETYDATA_API_KEY              (필수)
 *   - SAFETYDATA_TSUNAMI_API_URL      (선택 — 부여받은 endpoint URL 다른 경우)
 *   - KAKAO_REST_API_KEY              (응답에 좌표가 빠진 항목 폴백 지오코딩용)
...
async function main() {
  const [
    ...
    import("../lib/disasters/safetydataTsunamiEvacuation.ts"),
  ...
  for (let p = startPage; p <= endPage; p += 1) {
    ...
    try {
      const raw = await fetchTsunamiEvacuationRaw({
        serviceKey,
        pageNo: p,
        numOfRows,
      });
      ...
      const normalized = parsed.items.map((rawItem, index) =>
        normalizeTsunamiEvacuationItem(rawItem, p, index)
      );
